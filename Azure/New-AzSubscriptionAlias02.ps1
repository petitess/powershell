...
$billingAccounts = 'xxxxxx-3b42-53ba-e814-421e30e28525:1346c60a-08c0-4567-a607-xxxxxxxxxxx'
...
$rbac = "Owner"
$devopsOrg = "https://dev.azure.com/karolsek0491"
$devopsPrj = "AzureIAC"
...
foreach ($sub in $names) {
    ...
    $newSub = az account alias create --name $sub --billing-scope $billingScope --display-name $sub --workload 'Production'

    #New-AzManagementGroupSubscription -GroupId "mg-sek-prod-01" -SubscriptionId $newSub.SubscriptionId
    az account management-group subscription add --name "mg-sek-prod-01" --subscription $newSub.SubscriptionId
